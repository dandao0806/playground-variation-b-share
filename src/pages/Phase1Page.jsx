...
// the deployed subpath (e.g. /playground-variation-b-share/logo.svg).
...
// Phase 1 uses 3 tiers per Figma: Price Conscious | Most Popular | Superior
// Coverage. "Most Popular" is styled with the peach/salmon secondary fill when
// selected; the other two pills are outlined black.
const tiers = [
  { id: 'basic', name: 'Price Conscious', price: '$85/mo', basePrice: 85 },
  { id: 'standard', name: 'Most Popular', price: '$115.07/mo', basePrice: 115.07 },
  { id: 'superior', name: 'Superior Coverage', price: '$250/mo', basePrice: 250 },
]
...
// Compact discount trigger — sparkle icon + "Score! You've got discounts!"
// blue link + chevron. Clicking expands an inline list of applied discounts
// below the link. Matches the Figma design's bottom-of-hero treatment.
function Phase1DiscountLink() {
  const [open, setOpen] = useState(false)
  return (
    <div className={`phase1-discount ${open ? 'open' : ''}`}>
      <button
        type="button"
        className="phase1-discount-trigger"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
      >
        {/* Discount icon — inlined from Consumer/PremiumPreview/Discount icon.svg
            so it renders regardless of asset-caching or CSP quirks. */}
        <svg
          className="phase1-discount-sparkle"
          width="16"
          height="16"
          viewBox="0 0 16 16"
          fill="none"
        ...
        >
          <path
            d="M8 0C9.125 0 10.125 0.65625 10.625 1.625C11.6562 1.28125 12.8438 1.53125 13.6562 2.34375C14.4688 3.15625 14.7188 4.34375 14.375 5.375C15.3438 5.875 16 6.875 16 8C16 9.15625 15.3438 10.1562 14.375 10.6562C14.7188 11.6875 14.4688 12.8438 13.6562 13.6562C12.8438 14.4688 11.6562 14.7188 10.625 14.4062C10.125 15.375 9.125 16 8 16C6.84375 16 5.84375 15.375 5.34375 14.4062C4.3125 14.7188 3.15625 14.4688 2.34375 13.6562C1.53125 12.8438 1.28125 11.6875 1.59375 10.6562C0.625 10.1562 0 9.15625 0 8C0 6.875 0.625 5.875 1.59375 5.375C1.28125 4.34375 1.53125 3.15625 2.34375 2.34375C3.15625 1.53125 4.3125 1.28125 5.34375 1.625C5.84375 0.65625 6.84375 0 8 0ZM8.625 4.40625C8.625 4.0625 8.34375 3.78125 8 3.78125C7.65625 3.78125 7.375 4.0625 7.375 4.40625V4.9375C7.125 5 6.84375 5.09375 6.59375 5.25C6.125 5.53125 5.71875 6 5.71875 6.71875C5.71875 7.375 6.125 7.8125 6.53125 8.0625C6.90625 8.28125 7.375 8.4375 7.78125 8.5625H7.8125C8.25 8.6875 8.59375 8.8125 8.8125 8.9375C9 9.0625 9.0625 9.15625 9.0625 9.28125C9.0625 9.5 8.96875 9.625 8.8125 9.71875C8.625 9.84375 8.34375 9.90625 8.03125 9.90625C7.625 9.875 7.25 9.75 6.78125 9.59375C6.71875 9.5625 6.625 9.53125 6.53125 9.5C6.21875 9.40625 5.875 9.5625 5.75 9.90625C5.65625 10.2188 5.8125 10.5938 6.15625 10.6875C6.21875 10.7188 6.28125 10.75 6.34375 10.75C6.65625 10.875 7 11 7.375 11.0625V11.625C7.375 11.9688 7.65625 12.25 8 12.25C8.34375 12.25 8.625 11.9688 8.625 11.625V11.0938C8.9375 11.0312 9.21875 10.9375 9.46875 10.7812C9.9375 10.4688 10.3125 9.96875 10.3125 9.28125C10.2812 8.59375 9.90625 8.15625 9.46875 7.875C9.09375 7.625 8.59375 7.5 8.1875 7.375C7.75 7.21875 7.40625 7.125 7.1875 7C7 6.875 6.96875 6.8125 6.96875 6.71875C6.96875 6.53125 7.03125 6.4375 7.21875 6.3125C7.40625 6.1875 7.71875 6.125 8 6.125C8.34375 6.15625 8.71875 6.21875 9.09375 6.3125C9.4375 6.40625 9.78125 6.21875 9.875 5.875C9.9375 5.53125 9.75 5.1875 9.40625 5.125C9.1875 5.03125 8.90625 4.96875 8.625 4.9375V4.40625Z"
            fill="#0078D6"
          />
        </svg>
        <span className="phase1-discount-label">
          Score! You've got discounts!
        </span>
        <ChevronIcon
          size={16}
          className={`phase1-discount-chevron ${open ? 'open' : ''}`}
        />
      </button>
      <div className="phase1-discount-body" aria-hidden={!open}>
        <div className="phase1-discount-list">
          {appliedDiscounts.map((d) => (
            <div key={d.bold} className="phase1-discount-item">
              <CircleCheckIcon />
              <span>
...
// Phase 1 tier pills — 3 pills laid out with the first two side-by-side and
// the third centred on its own row. "Most Popular" (id=standard) is styled
// with a peachy secondary tint + orange border when selected; the other two
// use an outlined black pill treatment.
function Phase1TierPills({ selected, onSelect }) {
  const isMostPopular = (id) => id === 'standard'
  const renderPill = (t) => {
    const isSelected = selected === t.id
    const isPopular = isMostPopular(t.id)
    return (
      <button
        key={t.id}
        type="button"
        ...
        className={[
          'phase1-tier-pill',
          isPopular ? 'phase1-tier-pill--popular' : 'phase1-tier-pill--outline',
          isSelected ? 'selected' : '',
        ]
          .filter(Boolean)
          .join(' ')}
  ...
  const price = tiers.find((t) => t.id === 'basic')
  const popular = tiers.find((t) => t.id === 'standard')
  const superior = tiers.find((t) => t.id === 'superior')
  return (
    <div className="phase1-tier-pills">
      <div className="phase1-tier-pills-row">
        {renderPill(price)}
        {renderPill(popular)}
      </div>
      <div className="phase1-tier-pills-row phase1-tier-pills-row--single">
        {renderPill(superior)}
      </div>
    </div>
  )
    ...
    return (
      <div className="pr-coverage-row open pr-coverage-row--mobile">
        ...
        <div className="pr-coverage-current-line">
          <span className="pr-current-label">Premium:</span>
        ...
        {rows &&
          rows.map((row) => (
            <CoverageRow
              ...
              wasRecentlyUpdated={
                committed[row.name] !== previousCommitted[row.name]
              }
              onSelect={onSelect}
              viewport={viewport}
            />
...
export default function Phase1Page() {
  ...
  const tierBase = useMemo(
    () => tiers.find((t) => t.id === selectedTier)?.basePrice ?? 0,
    [selectedTier],
  )
  ...
  const handleRecalculate = () => {
    ...
    triggerFeedback(prev, computeMonthly(draftSelections, tierBase))
  }
  // Selecting a tier resets coverage selections to defaults so the big price
  // exactly matches that tier's headline number until the user customizes again.
    ...
    <div className="pr-shell" ref={shellRef}>
      ...
      <div className="pr-content">
        <div className="pr-hero">
          <h1 className="pr-hero-title pr-hero-title--inline">
            <span className="pr-hero-title-text">Your</span>
            <button type="button" className="pr-policy-term-select">
              <span className="pr-policy-term-value">6 month</span>
              <svg
            ...
            <span className="pr-hero-title-text">policy</span>
          </h1>
            ...
            return (
              <div className="phase1-payment">
                ...
                <div className="phase1-payment-savings">
                  Save <strong>${fmt(savings)}</strong> when you pay in full: $
                  {fmt(payInFull)}
                </div>
              </div>
            )
          ...
          <Button
            ...
            shape="pill"
            block
            className="phase1-cta"
            onClick={ctaOnClick}
          >
            {isPending ? 'Recalculate' : 'Continue to purchase'}
          </Button>

          <div className="phase1-quote-links">
            <a href="#email" className="phase1-quote-btn">
              <svg
                width="18"
                height="18"
                viewBox="0 0 18 18"
            ...
            <a href="#print" className="phase1-quote-btn">
              <svg
                width="18"
                height="18"
                viewBox="0 0 24 24"
        ...
        <CoverageSection
          ...
          previousCommitted={previousCommittedSelections}
        ...
        >
          ...
          <CoverageRow
            ...
            wasRecentlyUpdated={
              committedSelections[packageUpgradeRow.name] !==
              previousCommittedSelections[packageUpgradeRow.name]
            }
            onSelect={handleSelectRow}
            viewport={viewport}
          />
          ...
          <CoverageRow
            ...
            wasRecentlyUpdated={
              committedSelections[accidentForgivenessRow.name] !==
              previousCommittedSelections[accidentForgivenessRow.name]
            }
            onSelect={handleSelectRow}
            viewport={viewport}
          />
