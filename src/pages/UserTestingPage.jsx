...
// Icons use the 4 existing SVGs in /public/icons/ as placeholders — each
// reused no more than twice — so a drop-in of new per-coverage icons can
// replace them one-by-one without touching structure.
const summaryCoverages = [
  { id: 'bi', name: 'Bodily Injury', value: '$50,000/$100,000', icon: '/icons/ut-shield.svg' },
  { id: 'pd', name: 'Property Damage', value: '$10,000', icon: '/icons/ut-property.svg' },
  { id: 'uim', name: 'Underinsured/Uninsured', value: '$50,000/$100,000', icon: '/icons/ut-medical.svg' },
  { id: 'comp', name: 'Comprehensive Damage', value: '$5,000', icon: '/icons/ut-comprehensive.svg' },
  { id: 'coll', name: 'Collision Damage', value: '$500', icon: '/icons/ut-car.svg' },
  { id: 'roadside', name: 'Roadside Assistance', value: 'Free', icon: '/icons/ut-device.svg', chip: true },
]
      ...
      <ul className="ut-summary-list">
        {summaryCoverages.map((c) => (
          <li key={c.id} className="ut-summary-row">
            ...
            {c.chip ? (
              <span className="ut-free-chip">{c.value}</span>
            ) : (
              <span className="ut-summary-value">{c.value}</span>
            )}
          </li>
        ))}
      </ul>
...
function CoverageBody({
  ...
  return (
    <>
      ...
      {/* Description is a button so it can be tapped to expand/collapse
          past the 2-line truncation. Mirrors Example 1's blurb pattern. */}
      <button
        type="button"
...
function EditGroup({ group, draft, committed, previous, onSelect, stagger = 0 }) {
  if (group.collapsible) {
    return (
      <section className="ut-group" style={{ '--ut-stagger': stagger }}>
        <h3 className="ut-group-title">{group.title}</h3>
        <p className="ut-group-desc">{group.description}</p>
        <div className="ut-vehicle-card">
          <Accordion
            defaultExpanded={true}
            className="ut-vehicle-accordion"
            headerLeft={<div className="ut-vehicle-name">{group.vehicleName}</div>}
          >
  ...
  return (
    <section className="ut-group" style={{ '--ut-stagger': stagger }}>
      <h3 className="ut-group-title">{group.title}</h3>
      <p className="ut-group-desc">{group.description}</p>
      <div className="ut-cov-stack">
        {group.rows.map((row, i) => (
          <div
            key={row.name}
            className="ut-cov-card-wrap"
            style={{ '--ut-stagger': stagger + i }}
          >
            <CoverageCard
              row={row}
              draftValue={draft[row.name] ?? row.selection}
...
function AddonsTab({ draft, committed, previous, onSelect }) {
  return (
    <div className="ut-addons">
      <section className="ut-group" style={{ '--ut-stagger': 0 }}>
        <h3 className="ut-group-title">Upgrade packages</h3>
        <p className="ut-group-desc">
          You can add extra services and protection to your policy.
        </p>
        <div className="ut-cov-stack">
          <div className="ut-cov-card-wrap" style={{ '--ut-stagger': 0 }}>
            <PackageIncludedCard />
          </div>
          <div className="ut-cov-card-wrap" style={{ '--ut-stagger': 1 }}>
            <div className="ut-cov-card">
              <CoverageBody
                row={packageUpgradeRow}
                draftValue={draft[packageUpgradeRow.name] ?? packageUpgradeRow.selection}
                committedValue={committed[packageUpgradeRow.name] ?? packageUpgradeRow.selection}
                wasRecentlyUpdated={
      ...
      <section className="ut-group" style={{ '--ut-stagger': 2 }}>
        <h3 className="ut-group-title">Perks</h3>
        <p className="ut-group-desc">
          Optional extras you can add to get more from your coverage.
        </p>
        <div className="ut-cov-stack">
          <div className="ut-cov-card-wrap" style={{ '--ut-stagger': 2 }}>
            <div className="ut-cov-card">
              <PerkRow
                row={perksRow}
...
export default function UserTestingPage() {
  ...
  const { trackRef, registerPanel } = useSwipeTabs(tabs, tab, setTab)
  ...
  return (
    <div className={`pr-frame pr-frame--${viewport} ut-frame`}>
      ...
      <div className="pr-shell" ref={shellRef}>
        ...
        <div className="pr-content ut-content">
          ...
          {/* Horizontal scroll-snap track: all three panels are rendered
              side-by-side so mobile swipe (or programmatic scroll from a tab
              click) can slide between them. Tab state is bound to the visible
              panel via IntersectionObserver in useSwipeTabs. */}
