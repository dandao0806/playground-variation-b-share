...
// Split "$95/mo" into ["95", "/mo"] so the card can render the numeric price
// large with a small "$" superscript and a small "/mo" suffix, matching the
// reference screenshot. Falls back gracefully for edge shapes like "$—/mo".
...
function TierTabs({ selected, onSelect }) {
  return (
    <div className="pr-tier-cards-wrap">
      <div className="pr-tier-cards" role="tablist">
        {tiers.map((t) => {
          const { amount, unit } = splitPrice(t.price)
          ...
          return (
            <button
              ...
              className={`pr-tier-card ${isSelected ? 'selected' : ''}`}
            >
              <div className="pr-tier-card-price">
                <span className="pr-tier-card-currency">$</span>
                <span className="pr-tier-card-amount">{amount}</span>
                {unit && <span className="pr-tier-card-unit">{unit}</span>}
              </div>
              <div className="pr-tier-card-name">{t.name}</div>
              {isSelected && (
                <svg
                  className="pr-tier-card-tail"
                  width="24"
                  height="12"
                  viewBox="0 0 24 12"
                  aria-hidden="true"
                >
                  {/* Filled downward V — same fill as the card interior. */}
                  <path d="M0 0 L24 0 L12 12 Z" fill="#F5FAFF" />
                  {/* Stroke only on the two slanted sides, joined at the tip.
                      No top stroke — the card's own bottom border sits directly
                      above and provides the horizontal line. */}
                  <path
                    d="M0 0 L12 12 L24 0"
                    fill="none"
...
export default function TierCardsPage() {
    ...
    <div className="pr-shell" ref={shellRef}>
      ...
      <div className="pr-content">
        <div className="pr-hero">
          ...
          <TierTabs selected={selectedTier} onSelect={handleSelectTier} />
