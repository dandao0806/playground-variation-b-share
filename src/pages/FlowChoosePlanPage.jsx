...
const tiers = [
  { id: 'standard', name: 'Recommended', price: '$115.07/mo', basePrice: 115.07 },
...
// Overview data per tier — powers both the tab switcher (name + price) and
// the highlighted card (price, tagline, coverage summary) beneath it.
const planOverview = {
  standard: {
    id: 'standard',
    name: 'Recommended',
    price: '$115.07/mo',
    tagline: 'Balanced coverage and affordable rates',
    coverages: [
      {
        label: 'Bodily Injury & Property Damage',
        value: '$50k / $100k / $20k',
      },
      { label: 'Personal Injury Protection', value: '$250,000' },
      { label: 'Comp & Collision deductibles', value: '$5,000 / $500' },
    ],
  },
  superior: {
    id: 'superior',
    name: 'Superior',
    price: '$250/mo',
    tagline: 'Comprehensive coverage for maximum protection',
    coverages: [
      {
        label: 'Bodily Injury & Property Damage',
        value: '$250k / $500k / $50k',
      ...
      { label: 'Personal Injury Protection', value: '$250,000' },
      { label: 'Comp & Collision deductibles', value: '$1,000 / $250' },
    ],
  },
}

function PlanTabSwitcher({ selectedId, onSelect }) {
  return (
    <div className="pr-plan-tabnav" role="tablist">
      {Object.values(planOverview).map((p) => {
        ...
        return (
          <button
            ...
            className={`pr-plan-tabnav-item ${isSelected ? 'selected' : ''}`}
          >
            <span className="pr-plan-tabnav-name">{p.name}</span>
            <span className="pr-plan-tabnav-price">{p.price}</span>
          </button>
        )
      })}
    </div>
...
function PlanOverviewCard({ plan }) {
  return (
    <div className="pr-plan-overview-card">
      <div className="pr-plan-overview-price">{plan.price}</div>
      <div className="pr-plan-overview-tagline">{plan.tagline}</div>
      <hr className="pr-plan-overview-divider" aria-hidden="true" />
      <ul className="pr-plan-overview-list">
        {plan.coverages.map((c) => (
          <li key={c.label} className="pr-plan-overview-item">
            <span className="pr-plan-overview-item-label">{c.label}</span>
            <span className="pr-plan-overview-item-value">{c.value}</span>
          </li>
        ))}
      </ul>
    </div>
...
function ChangePlanConfirmModal({ open, onCancel, onConfirm }) {
  if (!open) return null
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="pr-change-plan-title"
      onClick={onCancel}
    >
      <div
        className="pr-modal pr-change-plan-modal"
      ...
      >
        ...
        <h2 id="pr-change-plan-title" className="pr-modal-title">
          Change your coverage package?
        </h2>
        <p className="pr-modal-body">
          Choosing a different package will reset all the customizations you've
          made to your current coverage. Are you sure you want to continue?
        </p>
...
function ChoosePlanScreen({
  ...
  // Screen 2 owns a local pending selection so a user can preview a different
  // pick before committing. Back/cancel just unmounts this component so the
  // shared plan state on the parent is left untouched.
  const [pending, setPending] = useState(currentTierId)
  ...
  return (
    <div className={`pr-frame pr-frame--${viewport}`}>
      ...
      <div className="pr-shell">
        ...
        <div className="pr-content pr-choose-content">
          <h1 className="pr-choose-title">Select your coverage.</h1>

          <PlanTabSwitcher selectedId={pending} onSelect={setPending} />
          <PlanOverviewCard
            plan={planOverview[pending] ?? planOverview.standard}
          />

          <div className="pr-choose-actions">
            <Button
              severity="primary"
              variant="outlined"
              size="large"
...
export default function FlowChoosePlanPage() {
  const [stepIndex, setStepIndex] = useState(4)
  // Shared plan state — persists across screen swaps. Default = Recommended,
  // labeled as the closest match on Screen 1.
  ...
  // Confirming a plan on Screen 2 resets coverage selections to defaults so
  // the big price on Screen 1 exactly matches the new plan's headline number
  // until the user customizes again.
  const handleConfirmPlan = (tierId) => {
  ...
  return (
    ...
    <div className="pr-shell" ref={shellRef}>
      ...
      <div className="pr-content">
        ...
        <div className="pr-choose-plan-cta-row">
          <Button
            severity="primary"
            size="large"
            shape="pill"
            variant="outlined"
            className="pr-choose-plan-cta"
            onClick={() => setShowChangePlanModal(true)}
          >
            Choose a different coverage plan
          </Button>
        </div>
